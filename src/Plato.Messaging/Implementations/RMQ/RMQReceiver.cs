...
namespace Plato.Messaging.Implementations.RMQ
{
    ...
    public class RMQReceiver : RMQReceiverSender, IMessageReceiver<byte[]>
    {
        ...
        public RMQReceiver(RMQSettings settings, IEnumerable<string> routingKeys = null) : base(settings)
        {
        ...
        public RMQReceiver(RMQSettings settings, string routingKey = "") : this(settings, new string[] { routingKey })
        {
        }
        ...
        public override void Open()
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        _channel.QueueBind(_settings.QueueSettings.Name, _settings.ExchangeSettings.Name, routingKey);
                    }
                }
                catch (Exception)
                {
        ...
        public IMessageReceiveResult<byte[]> Receive(int msecTimeout = Timeout.Infinite)
        {
            try
            {
                ...
                if (status)
                {
                    return new RMQReceiverResult(_connection, _channel, deliverArgs, _settings.QueueSettings.Name);
                }
